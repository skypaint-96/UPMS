...
public static class CanonicalFieldAliasRegistry
{
    ...
    public static IReadOnlyList<string> GetAliases(string fieldName)
    {
        ...
        if (AliasMap.TryGetValue(fieldName.Trim(), out var aliases))
            return aliases;

        foreach (var pair in AliasMap)
        {
            if (pair.Value.Any(alias => string.Equals(alias, fieldName.Trim(), StringComparison.OrdinalIgnoreCase)))
                return pair.Value;
        }
    ...
    public static bool Matches(string registeredName, string candidate)
    {
        if (string.IsNullOrWhiteSpace(registeredName) || string.IsNullOrWhiteSpace(candidate))
            return false;

        foreach (var alias in GetAliases(registeredName))
        {
            if (string.Equals(alias, candidate.Trim(), StringComparison.OrdinalIgnoreCase))
                return true;
        }
    ...
    public static string? ResolveRegisteredName(IEnumerable<string> registeredNames, string candidate)
    {
        ...
        var exact = names.FirstOrDefault(name => string.Equals(name, candidate.Trim(), StringComparison.OrdinalIgnoreCase));
